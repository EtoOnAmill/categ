...
void* inner_main(struct InnerArgs* inner_args) {
    printf("Home dir INNER: %s\n", home_dir);
    printf("User INNER: %s\n", user);
    printf("Categ dir INNER: %s\n", categ_dir);
    printf("Db name INNER: %s\n", db_name);
    ...
    append(&load_categ, "\")");
    append(&load_categ, "(use-modules (categ))");
...
int main(int argc, char** argv) {
    ...
    categ_dir = get_env("CATEG_DIR", xdg_data_home);
